...
from .services import kategoryzuj_z_gemini
...
def handle_kategorie():
    if request.method == 'GET':
        try:
            # ZMIANA: Pobieramy kategorie z dwóch źródeł
            ...
            for kat_tuple in kategorie_z_transakcji:
                ...
                if nazwa_kat and nazwa_kat not in kategorie_z_tabeli:
                    # To jest kategoria "ad-hoc", której nie ma w tabeli kategorii.
                    # Możemy ją zignorować lub dodać do listy (na razie ignorujemy, by nie zaśmiecać)
                    # W przyszłości można dodać logikę do zarządzania nimi.
                    pass
            
            # Na razie zwracamy tylko te "oficjalne" kategorie
...
def handle_transakcje():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # ZMIANA: Przekazujemy do AI listę istniejących kategorii
...
def handle_jedna_transakcja(transakcja_id):
    ...
    if request.method == 'PUT':
        ...
        try:
            # ZMIANA: Jeśli użytkownik zmienił opis, prosimy AI o nową kategorię na podstawie zdefiniowanej listy
...
def financial_summary():
    try:
        ...
        miesieczne_dane = defaultdict(lambda: { 'przychody': 0.0, 'wydatki': 0.0, 'wydatkiKategorie': defaultdict(float) })
        ...
        for miesiac in posortowane_miesiace:
            ...
            # ZMIANA: Dodajemy nowe pole `bilansMiesiaca`
            ...
            finalne_podsumowanie[miesiac] = {
                ...
                'bilansMiesiaca': round(bilans_miesiaca, 2), # <-- NOWE POLE
...
# Endpointy /limity i /szablony bez zmian
